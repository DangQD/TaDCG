...
class SingleHeadAttentionLayer(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        output = torch.sum(torch.unsqueeze(score, dim=-1) * value, dim=-2) + value
        return output
...
class DotProductAttentionOther(nn.Module):
    def __init__(self, value_size, other_event_size, attention_size):
        super().__init__()
        self.attention_size = attention_size
        self.context = nn.Parameter(data=nn.init.xavier_uniform_(torch.empty(attention_size, 1)))
        self.dense1 = nn.Linear(value_size+other_event_size, attention_size)

    def forward(self, x, y):
        t = self.dense1(torch.cat((x, y), dim=-1))
        vu = torch.matmul(t, self.context).squeeze()
        score = torch.softmax(vu, dim=-1)
        output = torch.sum(x * torch.unsqueeze(score, dim=-1), dim=-2)
        return output
...
class DotProductAttentionVisit(nn.Module):
    def __init__(self, value_size, attention_size):
        super().__init__()
        self.attention_size = attention_size
        self.context = nn.Parameter(data=nn.init.xavier_uniform_(torch.empty(attention_size, 1)))
