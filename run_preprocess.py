...
if __name__ == '__main__':
    conf = {
        ...
        'mimic4': {
            'parser': Mimic4Parser,  # HiTANet数据集划分是75:10:15
            'train_num': 7500,
            'test_num': 1500,
            'threshold': 0.01,
            'sample_num': 10000
    ...
    disease = 'copd'  # hf, copd, kd
    hf = ['428']
    # Chronic Obstructive Pulmonary Disease(COPD)
    # as a common chronic respiratory disease is the general term for chronic bronchitis (支气管炎) and emphysema (肺气肿)
    copd = ['490', '491', '492', '496']
    kd = ['584', '585', '586']
    disease_prefix = eval(disease)
    ...
    (train_code_x, train_codes_y, train_visit_lens, train_visit_intervals) = build_code_xy(train_pids, *common_args)
    print('building valid codes features and labels ...')
    (valid_code_x, valid_codes_y, valid_visit_lens, valid_visit_intervals) = build_code_xy(valid_pids, *common_args)
    print('building test codes features and labels ...')
    (test_code_x, test_codes_y, test_visit_lens, test_visit_intervals) = build_code_xy(test_pids, *common_args)
    ...
    print('building train %s labels ...' % disease)
    train_hf_y = build_heart_failure_y(disease_prefix, train_codes_y, code_map)
    print('building valid %s labels ...' % disease)
    valid_hf_y = build_heart_failure_y(disease_prefix, valid_codes_y, code_map)
    print('building test %s labels ...' % disease)
    test_hf_y = build_heart_failure_y(disease_prefix, test_codes_y, code_map)
    ...
    save_data(train_path, train_code_x, train_visit_lens, train_codes_y, train_hf_y, train_divided, train_neighbors, train_visit_intervals, disease)
    print('\tsaving valid data')
    save_data(valid_path, valid_code_x, valid_visit_lens, valid_codes_y, valid_hf_y, valid_divided, valid_neighbors, valid_visit_intervals, disease)
    print('\tsaving test data')
    save_data(test_path, test_code_x, test_visit_lens, test_codes_y, test_hf_y, test_divided, test_neighbors, test_visit_intervals, disease)
